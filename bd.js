...
async function updateSaida(id, container) {
  return updateItem('saidas', id, container);
}
async function deleteSaida(id) {
  return deleteItem('saidas', id);
}

// Função para limpar coleções inteiras (usada no histórico)
async function clearCollection(path) {
    await writeToFirebase(path, null);
}
